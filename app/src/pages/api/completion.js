...
const AI_PROMPT = "The folling is a conversation with ZzRose. ZzRose is helpful and creative. ZzRose's only knowledge is React JS library. ZzRose can only answer questions related to React JS. He only cares about React JS. ZzRose provides often code examples. ZzRose provides answers formated in markdown format."
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      const completion = await openai.createCompletion({
        ...
        prompt: aiPrompt + db.data.messageHistory[user.uid].join('') + "ZzRose:",
        temperature: 0.7,
        max_tokens: 1024
      })
